...
#######################################################################
# Model
path = "/data1/pengrui/Model/OpenGVLab/InternVL2_5-1B/"
pretrained_pth = None
...
# Keep DataLoader light to avoid host OOM and worker kills
...
model = dict(
    ...
    grounding_encoder=dict(
        ...
        enable_cma=True,  # enable cross-modal attention injection
        cma_num_heads=8,
    ),
    loss_mask=dict(
...
DATA_ROOT = "/data1/pengrui/CodeSpace/Sa2VA/data/Sa2VA-Training/"
...
data_root_refytvos = VIDEO_DATA_ROOT + "davis17/"
...
# sam2 data
data_sam2_folder = DATA_ROOT + "sav_train/"
data_sam2_expression_file = data_sam2_folder + "Ref-SAV.json"
...
data_osprey_detail_description_file = (
    OSPREY_ROOT + "Osprey-724K/osprey_detail_description.json"
)
...
data_osprey_positive_neg_file = (
    OSPREY_ROOT + "Osprey-724K/osprey_lvis_positive_negative.json"
)
...
custom_hooks = [
    ...
    dict(
        ...
        capture_stderr=True,
        log_env_info=True,
        log_cfg_snapshot=False,
    ),
    # Show fast-forwarding progress when resuming
    dict(type=ResumeProgressHook, log_every=500),
]
...
default_hooks = dict(
    ...
    checkpoint=dict(
        type=CheckpointHook,
        save_optimizer=True,  # save optimizer (and AMP scaler) states for resume
        save_param_scheduler=True,  # also save LR scheduler state
        save_last=True,  # keep a pointer to latest checkpoint for auto-resume
        by_epoch=False,
        interval=save_steps,
...
env_cfg = dict(
    ...
    # Use spawn to avoid CUDA/fork pitfalls and improve DataLoader stability
...
resume = True  # auto-resume from latest checkpoint in work_dir
